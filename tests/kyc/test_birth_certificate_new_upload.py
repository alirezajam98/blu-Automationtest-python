...
# Behaviors
@allure.epic("KYC")
@allure.feature("Create a new user account - New Birth Certificate Upload")
@allure.story("Complete account creation by uploading new birth certificate")
# Suites
@allure.suite(f"version:{VERSION}")
@allure.sub_suite("Tests for New Birth Certificate Upload")
@allure.severity(allure.severity_level.BLOCKER)
# Links
@allure.link("https://jira.sdb247.com/browse/", name="Jira Task")
def test_birth_certificate_new_upload(open_app_without_login):
    ...
    try:
        ...
        with allure.step("Select 'birth_certificate_new' option"):
            ...
            select_type_cert.click_birth_certificate_new()
            logger.info("گزینه 'شناسنامه جدید' انتخاب شد.")
        try:
