from utils.kyc_panel import run_main_process
import json
...
from pages.kyc.kyc_pages import NotificationPermissionPage, CreateAccountInBluStatePage, \
    SelectNationalCardOrTrackerIdPage, \
    TakePhotoPage, ConfirmPhotoPage, ConfirmPhotoModal, SelectJobPage, VideoDemoPage, \
    VideoRecordingPage, ConfirmVideoModal, FinalPage, GalleryPage, EditPhotoPage
...
# Behaviors
@allure.epic("KYC")
@allure.feature("Create a new user account - National ID Upload")
@allure.story("Complete account creation by uploading National ID")
# Suites
@allure.suite(f"version:{VERSION}")
@allure.sub_suite("Tests for National ID Upload")
...
def test_national_id_card_upload(open_app_without_login):
    ...
    with open("utils/generated_username.txt", "w") as f:
        f.write(username)
    # اجرای مراحل مشترک
    ...
    try:
        ...
        with allure.step("select gallery button"):
            ...
            take_photo_page.click_select_from_gallery()
        ...
        with allure.step("click on first image from gallery"):
            ...
            gallery_page.click_fourth_photo()
            logger.info("روی اولین عکس گالری برای تصویر جلوی کارت ملی کلیک شد.")
        ...
        with allure.step("Take photo from back of national card"):
            ...
            gallery_page.click_album_button()
            gallery_page.click_favorite_album()
            gallery_page.click_third_photo()
